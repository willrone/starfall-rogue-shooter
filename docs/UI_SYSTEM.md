...
- 正式加载图、分享封面和应用图标位于 `assets/app_store/`，使用克制中等饱和的扁平卡通风格；推广图不能代替 720×1280 当前构建的提审实机截图。
...
- 65 个 RUN_ITEM 专属图标尚未接入 6 格商店，商店仍使用两种通用属性图。
- `loadIcons()` 异步完成后只写缓存，不刷新已经打开的机库、副武器和熔炉面板；部分弹窗初建时没有 Icon 节点，后续也不能补建。
- 17 把主武器虽都有场上资源，但目前复用 UI 方形图标，独立手持构图和逐把实机验收未完成。
- 15 把副武器的 PNG 只接到玩家身边的身份徽章，真实机制实体仍主要由 Graphics/VFX 绘制。
- `ui/materials/`、`ui/tabs/` 为空，部分 HUD、面板背景和机制特效仍使用程序化 Graphics。
- 首页、商店、升级、暂停、复活、结算及全量装备/Boss 尚无同一候选构建的 720×1280 视觉验收证据。
