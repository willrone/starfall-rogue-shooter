...
def trigger_shop(cdp: CDPClient) -> None:
    ...
    try:
        cdp.evaluate(
            "(function(){try{var g=window.__starfallGame;if(g.shop&&g.shop.openShop)g.shop.openShop();return'ok'}catch(e){return e.message}})()",
            timeout=3,
        )
    except Exception:
...
def tick_real_game(cdp: CDPClient, frames: int, max_retries: int = 3) -> int:
    ...
    """
    frames_done = 0
    batch = 300  # 5s game time per CDP call — reduces WebSocket overhead 15x
    for attempt in range(max_retries):
        try:
            while frames_done < frames:
                rem = min(batch, frames - frames_done)
                # timeout: generous for late-game computation spikes
                t = max(25, rem / 10 + 5)
                result = cdp.evaluate(
