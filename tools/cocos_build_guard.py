...
PROJECT_CONFIG = OUTPUT / 'project.config.json'
BYTEDANCE_PROJECT = PROJECT / 'config/bytedance-project.json'
...
def apply_project_identity():
    """Replace Cocos's test AppID in the generated ByteDance project."""
    if not PROJECT_CONFIG.exists():
        return
    if not BYTEDANCE_PROJECT.exists():
        raise SystemExit(f'missing {BYTEDANCE_PROJECT.relative_to(PROJECT)}')

    identity = json.loads(BYTEDANCE_PROJECT.read_text(encoding='utf-8'))
    appid = str(identity.get('appid', '')).strip()
    projectname = str(identity.get('projectname', '')).strip()
    if not appid.startswith('tt') or not projectname:
        raise SystemExit('invalid config/bytedance-project.json')

    generated = json.loads(PROJECT_CONFIG.read_text(encoding='utf-8'))
    generated['appid'] = appid
    generated['projectname'] = projectname
    PROJECT_CONFIG.write_text(
        json.dumps(generated, ensure_ascii=False, separators=(',', ':')),
        encoding='utf-8',
    )
    print(f'Applied ByteDance project identity: {projectname} ({appid})')
